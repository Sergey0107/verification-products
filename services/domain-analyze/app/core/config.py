...
class Settings(BaseSettings):
    ...
    # Чанки независимы (разные наборы characteristics, никакого общего
    # состояния между вызовами _compare_chunk) — раньше обрабатывались строго
    # последовательно, хотя каждый чанк — I/O-bound HTTP-запрос к LLM
    # (36-225 сек на реальных документах), не CPU-bound работа. На документе
    # с 8 чанками последовательная обработка растягивала сравнение на
    # 25-30+ минут при том, что запросы друг другу не мешают. Ограничение
    # параллелизма — чтобы не упереться в rate-limit провайдера (уже
    # наблюдались 429 от Yandex на OCR-этапе того же аккаунта/квоты).
    #
    # Поднято с 3 до 8 (см. обсуждение с пользователем 2026-08-12): сравнение
    # сейчас идёт через AI Tunnel/qwen3.7-flash — лёгкая, быстрая модель без
    # тяжёлого Yandex reasoning-режима, для которого изначально был выбран
    # консервативный лимит 3. На анализах с 200+ характеристик (5+ чанков по
    # 50) лимит в 3 растягивал сравнение на две последовательные волны вместо
    # одной. При регрессии/429 от AI Tunnel — можно снизить обратно через env.
    COMPARE_CHUNK_CONCURRENCY: int = 8
