...
async def files_callback(payload: dict, db: AsyncSession = Depends(get_db)):
    ...
    if uploaded_count >= 2 and status_before != "files_uploaded":
        ...
        # Сразу извлекается ТОЛЬКО ТЗ. Паспорт ждёт одобрения характеристик
        # ТЗ пользователем и запускается из continue_tz_review (analyses.py).
        #
        # Раньше оба файла шли параллельно — быстрее, но паспорт извлекался
        # «вслепую»: список одобренных характеристик ТЗ в этот момент ещё не
        # существует, а именно он передаётся в промпт паспорта как контракт
        # «верни ровно эти имена дословно» (target_characteristics). Без него
        # модель придумывает свои формулировки, и сопоставление ТЗ↔паспорт
        # становится вероятностным вместо прямого — замер на ДЖАМБО 60/35
        # показал 7 из 15 требований ТЗ, дошедших до таблицы сравнения, при
        # 117 строках мусора в ней.
