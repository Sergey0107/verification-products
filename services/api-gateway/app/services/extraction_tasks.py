...
def _build_target_characteristics_appendix(
    ...
    if product_model:
        ...
        model_hint = (
            ...
            "'ДЖАМБО 60/35, напор 5 м' — that is ONE model 'ДЖАМБО 60/35'. Values for different "
            "operating points become SEPARATE CHARACTERISTICS inside it, with the condition moved "
            "into the characteristic name ('Подача при напоре 0 м' = 60 л/мин, 'Подача при напоре "
            "5 м' = 55 л/мин). product_name must contain ONLY the product designation — no commas, "
            ...
            "- MOTOR SELECTION TABLES (подбор двигателя): a passport often has a table that lists "
            "SEVERAL motor/power options for ONE pump model — one per liquid density range "
            "(напр. 'до 0,8', 'св.0,8 до 1', 'св.1 до 1,2', 'св.1,2 до 1,85'), giving e.g. "
            "55/75/90 kW for the SAME model. Do NOT blindly take the FIRST column. Either pick the "
            "option matching the density required by the requirement, or — if density is unknown — "
            "prefer the pump's OWN 'Потребляемая мощность' from the main parameters table "
            "(Подача/Напор/Мощность) over a selected motor power. Never present one density-variant "
            "motor power as if it were the unique characteristic of the pump.\n"
            "- POWER: 'Потребляемая мощность насоса' (pump consumed power, in the main "
